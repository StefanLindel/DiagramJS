...
export default class SymbolLibary {
  ...
  public static createGroup(node: BaseElement, group) {
    ...
    for (i = 0; i < group.items.length; i += 1) {
      svg.appendChild(create(group.items[i]));
    }

    let elements = node['elements'];
    setSize(svg, group.width + node.getSize().x, group.height + node.getSize().y);
    ...
    svg.close = function() {
      ...
      // typ.setSize(g, g.tool.width + g.tool.x, g.tool.height + g.tool.y);
      setSize(svg, svg.tool.size.x, svg.tool.size.y);
    };
    ...
    svg.open = function() {
      ...
      setSize(svg, svg.tool.width, svg.tool.height);
      // typ.setSize(g, g.tool.width + g.tool.x + 10, g.tool.height + g.tool.y + 10);
    };
    svg.close();
